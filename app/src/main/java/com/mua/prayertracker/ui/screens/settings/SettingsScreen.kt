...
/**
 * Card for viewing and manually overriding the location used for prayer
 * time calculation. A manually set location is persisted and takes
 * precedence over the device location until cleared.
 */
@Composable
private fun ManualLocationCard(viewModel: PrayerTrackerViewModel) {
	...
	var latitudeInput by remember(manualLocation) {
		mutableStateOf(manualLocation?.latitude?.toString() ?: "")
	}
	var longitudeInput by remember(manualLocation) {
		mutableStateOf(manualLocation?.longitude?.toString() ?: "")
	}
	...
	Card(
		...
		Column(
			...
			Text(
				text = when {
					manualLocation != null -> "Manually set: ${manualLocation?.formatted()}"
					deviceLocation != null -> "Device location: ${deviceLocation?.formatted()}"
					else -> "No location available yet"
				},
			...
			if (manualLocation != null && deviceLocation != null) {
				formatDistance(distanceBetweenMeters(manualLocation, deviceLocation))?.let {
					Text(
						text = "$it from device location",
			...
			Row {
				TextButton(onClick = {
					showInvalidInput =
						!viewModel.setManualLocation(latitudeInput, longitudeInput)
				}) {
					Text("Set manually")
				}

				if (manualLocation != null) {
					TextButton(onClick = { viewModel.clearManualLocation() }) {
